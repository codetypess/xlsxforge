...
function getEnumMemberName(member: ts.EnumMember, sourceFile: ts.SourceFile): string {
...
function getEnumMemberValue(
    ...
    autoValue: number,
    sourceFile: ts.SourceFile
): { text: string; nextAuto: number } {
    if (!member.initializer) {
        return { text: String(autoValue), nextAuto: autoValue + 1 };
    }
    const init = member.initializer;
    if (ts.isNumericLiteral(init)) {
        const n = Number(init.text);
        return { text: init.text, nextAuto: n + 1 };
    }
    if (
        ...
        return { text: `-${init.operand.text}`, nextAuto: n + 1 };
    }
    if (ts.isStringLiteral(init)) {
        return { text: JSON.stringify(init.text), nextAuto: autoValue };
    }
    return { text: init.getText(sourceFile), nextAuto: autoValue + 1 };
}

/**
 * Inline enum members instead of `z.enum(SomeEnum)` + importing the TS enum.
 * `validateJson` uses native `import(fileURL)`, and TypeScript `export enum`
 * is not a reliable ESM named export in that path (Node/tsx type stripping).
 */
function emitEnumSchema(stmt: ts.EnumDeclaration, ctx: GenContext): string[] {
    const name = stmt.name.text;
    const lines = [`export const ${toSchemaName(name)} = z.enum({`];
    let auto = 0;
    for (const member of stmt.members) {
        const key = getEnumMemberName(member, ctx.sourceFile);
        const quotedKey = /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(key) ? key : JSON.stringify(key);
        const { text, nextAuto } = getEnumMemberValue(member, auto, ctx.sourceFile);
        auto = nextAuto;
        lines.push(`    ${quotedKey}: ${text},`);
    }
    lines.push(`});`);
    return lines;
}
...
export async function tsToZod(inputPath: string, outputPath: string): Promise<void> {
    ...
    const output = assembleOutput(ctx, schemaLines, reExportPaths);
